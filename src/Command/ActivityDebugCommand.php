...
final class ActivityDebugCommand extends Command
{
     ...
     * @param list<class-string>              $activitiesWithoutWorkers
     */
    ...
    protected function execute(Input $input, Output $output): int
    {
        ...
        foreach ($this->workers as $name => $worker) {
            ...
            foreach ($worker->getActivities() as $activity) {
                if (in_array($activity->getClass()->name, $this->activitiesWithoutWorkers, true)) {
                    continue;
                }
        ...
        if (empty($this->activitiesWithoutWorkers) || !empty($workers)) {
            return self::SUCCESS;
        }
        ...
        foreach ($this->workers as $worker) {
            ...
            foreach ($worker->getActivities() as $activity) {
                ...
                if (in_array($activity->getClass()->name, $printedActivities, true)) {
                    continue;
                }
